...
public class VersionController {
    ...
    @PostMapping("/create")
    public ResponseEntity<VersionDTOResponse> createVersion(@RequestBody VersionDTO versionDTO){
        return new ResponseEntity<>(service.createVersion(versionDTO), HttpStatus.CREATED);
    }
    @PutMapping("/update/{id}")
    public ResponseEntity<VersionDTOResponse> updateVersion(@RequestBody VersionDTO versionDTO, @PathVariable Long id){
